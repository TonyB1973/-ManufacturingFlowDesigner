...
const legacy = createDemoProject(); const legacyDocument = { ...legacy, schemaVersion: '1.4.0', applicationVersion: '0.6.0', operationTemplates: legacy.operationTemplates.map((item) => ({ ...item, timingCategory: 'Value Added' })), operations: legacy.operations.map((item) => ({ ...item, timingCategory: 'Value Added' })) }; delete legacyDocument.standardWorkStudies; delete legacyDocument.standardWorkEntries; delete legacyDocument.standardWorkOperators; delete legacyDocument.standardWorkHandovers; delete legacyDocument.standardWorkPlanning; const migrated = deserializeProject(JSON.stringify(legacyDocument)); assert(migrated.migratedFrom === '1.4.0' && migrated.document.schemaVersion === '1.9.0' && migrated.document.standardWorkStudies.length === 0 && migrated.document.settings.standardWork.timeFormat === 'seconds' && migrated.document.settings.standardWork.chart.showAutomaticLanes, 'Schema 1.4 migrates explicitly through availability-capable Standard Work defaults');
