...
const standardWorkPlanning = new StandardWorkPlanningStore((id) => Boolean(standardWork.getStudy(id)));
...
const noopStore = {}; const context = { resources, operations, connections, structure: noopStore, routes: noopStore, annotations: noopStore, standardWork, standardWorkOperators, standardWorkHandovers, standardWorkPlanning, standardWorkSelection: standardSelection, project, selection: canvasSelection };
...
const legacy = createDemoProject(); const { activeScenarioId: _activeScenarioId, scenarios: _scenarios, ...legacyShared } = legacy; const legacyState = legacy.scenarios[0].state; const legacyDocument = { ...legacyShared, ...legacyState, schemaVersion: '1.4.0', applicationVersion: '0.6.0', operationTemplates: legacy.operationTemplates.map((item) => ({ ...item, timingCategory: 'Value Added' })), operations: legacyState.operations.map((item) => ({ ...item, timingCategory: 'Value Added' })) }; delete legacyDocument.standardWorkStudies; delete legacyDocument.standardWorkEntries; delete legacyDocument.standardWorkOperators; delete legacyDocument.standardWorkHandovers; delete legacyDocument.standardWorkPlanning; const migrated = deserializeProject(JSON.stringify(legacyDocument)); const migratedState = migrated.document.scenarios[0].state; assert(migrated.migratedFrom === '1.4.0' && migrated.document.schemaVersion === '2.0.0' && migratedState.standardWorkStudies.length === 0 && migrated.document.settings.standardWork.timeFormat === 'seconds' && migrated.document.settings.standardWork.chart.showAutomaticLanes, 'Schema 1.4 migrates explicitly through availability and the scenario envelope');
