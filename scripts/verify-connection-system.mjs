...
const operation = (id, sequence, x) => ({ id, templateId: 'op-cut', name: id, operationType: 'Fabrication', timingCategory: 'Value Added', cycleTimeSeconds: 30, sequence, assignedResourceId: null, notes: '', worldX: x, worldY: 0, width: 100, height: 60, selected: false, locked: false, visible: true });
...
selection.select({ kind: 'resource', id: 'RES-1' }); assert(selection.getSelection().kind === 'resource', 'Resource selection kind is supported'); selection.select({ kind: 'operation', id: 'OP-A' }); assert(selection.getSelection().kind === 'operation', 'Operation selection kind is supported'); store.selectConnection(created.connection.id); assert(selection.getSelection().kind === 'connection' && created.connection.selected, 'Connection selection clears other kinds'); selection.clear(); assert(selection.getSelection().kind === 'none', 'None selection kind is supported');
