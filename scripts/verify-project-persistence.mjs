...
const migratedOneSix = deserializeProject(JSON.stringify(schemaOneSix)); assert(migratedOneSix.migratedFrom === '1.6.0' && migratedOneSix.document.schemaVersion === '1.7.0' && migratedOneSix.document.standardWorkOperators.length === 1 && migratedOneSix.document.standardWorkEntries[0].assignedOperatorId === migratedOneSix.document.standardWorkOperators[0].id, 'Schema 1.6 projects gain deterministic default operators and entry assignments');
...
const output = serializeProject({ metadata: session.getMetadata(), settings: session.getSettings(), resources, operations, connections, structure, routes, annotations, standardWork: session.standardWork, standardWorkOperators: session.standardWorkOperators, standardWorkHandovers: session.standardWorkHandovers, workspaces }, '2026-01-03T00:00:00.000Z');
...
assert(migrated.migratedFrom === '0.9.0' && migrated.document.schemaVersion === '1.7.0', 'Registered older schema migrations chain into the current schema');
...
const emptyOutput = serializeProject({ metadata: session.getMetadata(), settings: session.getSettings(), resources, operations, connections, structure, routes, annotations, standardWork: session.standardWork, standardWorkOperators: session.standardWorkOperators, standardWorkHandovers: session.standardWorkHandovers, workspaces }); assert(emptyOutput.document.resources.length === 0 && emptyOutput.document.operations.length === 0 && emptyOutput.document.connections.length === 0 && emptyOutput.document.walls.length === 0 && emptyOutput.document.factoryRoutes.length === 0 && emptyOutput.document.factoryAnnotations.length === 0 && emptyOutput.document.standardWorkEntries.length === 0, 'Empty project serializes completely');
