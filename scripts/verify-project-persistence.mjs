...
const migratedOneSix = deserializeProject(JSON.stringify(schemaOneSix)); assert(migratedOneSix.migratedFrom === '1.6.0' && migratedOneSix.document.schemaVersion === '1.8.0' && migratedOneSix.document.standardWorkOperators.length === 1 && migratedOneSix.document.standardWorkPlanning.length === 1 && !migratedOneSix.document.standardWorkPlanning[0].active && migratedOneSix.document.standardWorkEntries[0].assignedOperatorId === migratedOneSix.document.standardWorkOperators[0].id, 'Schema 1.6 projects gain deterministic default operators, assignments, and inactive planning');
const migratedOneSeven = deserializeProject(JSON.stringify(schemaOneSeven)); assert(migratedOneSeven.migratedFrom === '1.7.0' && migratedOneSeven.document.schemaVersion === '1.8.0' && migratedOneSeven.document.standardWorkPlanning.length === 1 && migratedOneSeven.document.standardWorkPlanning[0].scheduledProductionTimeSeconds === 28800 && !migratedOneSeven.document.standardWorkPlanning[0].active, 'Schema 1.7 projects gain one inactive default planning record per study');
...
assert(migrated.migratedFrom === '0.9.0' && migrated.document.schemaVersion === '1.8.0', 'Registered older schema migrations chain into the current schema');
