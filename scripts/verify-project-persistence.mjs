...
const migratedOneSix = deserializeProject(JSON.stringify(schemaOneSix)); assert(migratedOneSix.migratedFrom === '1.6.0' && migratedOneSix.document.schemaVersion === '1.9.0' && migratedOneSix.document.standardWorkOperators.length === 1 && migratedOneSix.document.standardWorkPlanning.length === 1 && !migratedOneSix.document.standardWorkPlanning[0].active && migratedOneSix.document.standardWorkEntries[0].assignedOperatorId === migratedOneSix.document.standardWorkOperators[0].id, 'Schema 1.6 projects gain deterministic default operators, assignments, inactive planning, and availability migration defaults');
const migratedOneSeven = deserializeProject(JSON.stringify(schemaOneSeven)); assert(migratedOneSeven.migratedFrom === '1.7.0' && migratedOneSeven.document.schemaVersion === '1.9.0' && migratedOneSeven.document.standardWorkPlanning.length === 1 && migratedOneSeven.document.standardWorkPlanning[0].scheduledProductionTimeSeconds === 28800 && !migratedOneSeven.document.standardWorkPlanning[0].active, 'Schema 1.7 projects gain one inactive default planning record per study and migrate through availability');
const migratedOneEight = deserializeProject(JSON.stringify(schemaOneEight)); assert(migratedOneEight.migratedFrom === '1.8.0' && migratedOneEight.document.schemaVersion === '1.9.0' && migratedOneEight.document.standardWorkPlanning[0].availabilityMode === 'manual' && migratedOneEight.document.standardWorkPlanning[0].scheduledProductionTimeSeconds === 28800 && migratedOneEight.document.availabilityCalendars.length === 0 && migratedOneEight.document.settings.defaultAvailabilityCalendarId === null, 'Schema 1.8 projects preserve manual planning and gain empty availability collections without invented calendars');
...
const output = serializeProject({ metadata: session.getMetadata(), settings: session.getSettings(), resources, operations, connections, structure, routes, annotations, standardWork: session.standardWork, standardWorkOperators: session.standardWorkOperators, standardWorkHandovers: session.standardWorkHandovers, standardWorkPlanning: session.standardWorkPlanning, availability: session.availability, workspaces }, '2026-01-03T00:00:00.000Z');
...
assert(migrated.migratedFrom === '0.9.0' && migrated.document.schemaVersion === '1.9.0', 'Registered older schema migrations chain into the current schema');
...
const emptyOutput = serializeProject({ metadata: session.getMetadata(), settings: session.getSettings(), resources, operations, connections, structure, routes, annotations, standardWork: session.standardWork, standardWorkOperators: session.standardWorkOperators, standardWorkHandovers: session.standardWorkHandovers, standardWorkPlanning: session.standardWorkPlanning, availability: session.availability, workspaces }); assert(emptyOutput.document.resources.length === 0 && emptyOutput.document.operations.length === 0 && emptyOutput.document.connections.length === 0 && emptyOutput.document.walls.length === 0 && emptyOutput.document.factoryRoutes.length === 0 && emptyOutput.document.factoryAnnotations.length === 0 && emptyOutput.document.standardWorkEntries.length === 0 && emptyOutput.document.shiftDefinitions.length === 0 && emptyOutput.document.availabilityCalendars.length === 0, 'Empty project serializes completely');
