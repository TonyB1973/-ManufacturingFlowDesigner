...
const fixture = { format: 'ManufacturingFlowDesigner', schemaVersion: '1.4.0', applicationVersion: '0.6.0', project: { id: 'PRJ-0047', name: 'Persistence Test', description: 'Round trip', author: 'Engineer', company: 'Factory', createdUtc: '2026-01-01T00:00:00.000Z', modifiedUtc: '2026-01-02T00:00:00.000Z' }, resourceTemplates: [resourceTemplate], operationTemplates: [operationTemplate], resources: [resource, resourceTwo, inspection], operations: [operationA, operationB, operationC, operationD], connections: [connection, connectionTwo, connectionThree], layoutBoundaries: [boundary], walls: [wall], areas: [area], aisles: [aisle], factoryRoutes: [factoryRoute], factoryAnnotations: [], workspaces: { active: 'factoryLayout', processFlow: viewport, factoryLayout: { ...viewport, panX: -500, zoom: 0.75 } }, settings: { gridBaseInterval: 20, routingClearance: 16, unitSystem: 'metric', displayPrecision: 2, units: { modelLengthUnit: 'mm', displayLengthUnit: 'mm', displayPrecision: 2, showTrailingZeros: true }, dimensionTextScale: 1, annotationTextSize: 16, defaultDimensionOffset: 40, defaultDimensionLayer: 'Dimensions' } };
...
const output = serializeProject({ metadata: session.getMetadata(), settings: session.getSettings(), resources, operations, connections, structure, routes, annotations, workspaces }, '2026-01-03T00:00:00.000Z');
...
assert(migrated.migratedFrom === '0.9.0' && migrated.document.schemaVersion === '1.4.0', 'Registered older schema migrations chain into the current schema');
...
const emptyOutput = serializeProject({ metadata: session.getMetadata(), settings: session.getSettings(), resources, operations, connections, structure, routes, annotations, workspaces }); assert(emptyOutput.document.resources.length === 0 && emptyOutput.document.operations.length === 0 && emptyOutput.document.connections.length === 0 && emptyOutput.document.walls.length === 0 && emptyOutput.document.factoryRoutes.length === 0 && emptyOutput.document.factoryAnnotations.length === 0, 'Empty project serializes completely');
