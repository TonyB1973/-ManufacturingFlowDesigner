...
const session = new ProjectSessionService(resources, operations, connections, structure, routes, workspaces, selection, resourceIds, operationIds, connectionIds, routeIds);
...
const output = serializeProject({ metadata: session.getMetadata(), settings: session.getSettings(), resources, operations, connections, structure, routes, workspaces }, '2026-01-03T00:00:00.000Z');
...
assert(migrated.migratedFrom === '0.9.0' && migrated.document.schemaVersion === '1.3.0', 'Registered older schema migrations chain into the current schema');
...
const emptyOutput = serializeProject({ metadata: session.getMetadata(), settings: session.getSettings(), resources, operations, connections, structure, routes, workspaces }); assert(emptyOutput.document.resources.length === 0 && emptyOutput.document.operations.length === 0 && emptyOutput.document.connections.length === 0 && emptyOutput.document.walls.length === 0 && emptyOutput.document.factoryRoutes.length === 0, 'Empty project serializes completely');
