...
const structure = new FactoryStructureStore(new FactoryStructureIdGenerator('BND'), new FactoryStructureIdGenerator('WALL'), new FactoryStructureIdGenerator('AREA'), new FactoryStructureIdGenerator('AISLE')); const session = new ProjectSessionService(resources, operations, connections, structure, workspaces, selection, resourceIds, operationIds, connectionIds);
const context = { resources, operations, connections, structure, project: session, selection }; const modelHistory = new CommandHistoryService(context, 200); session.attachHistory(modelHistory); const commands = new CommandFactory(modelHistory, context);
...
const document = { format: 'ManufacturingFlowDesigner', schemaVersion: '1.2.0', applicationVersion: '0.4.0', project: { ...session.getMetadata(), name: 'Opened Project' }, resourceTemplates: RESOURCE_TEMPLATES, operationTemplates: OPERATION_TEMPLATES, resources: [], operations: [], connections: [], layoutBoundaries: [], walls: [], areas: [], aisles: [], workspaces: { active: 'processFlow', processFlow: workspaces.getViewport('processFlow'), factoryLayout: workspaces.getViewport('factoryLayout') }, settings: session.getSettings() };
