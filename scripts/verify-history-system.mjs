...
const structure = new FactoryStructureStore(new FactoryStructureIdGenerator('BND'), new FactoryStructureIdGenerator('WALL'), new FactoryStructureIdGenerator('AREA'), new FactoryStructureIdGenerator('AISLE')); const routeIds = new FactoryRouteIdGenerator(); const routes = new FactoryRouteStore(routeIds, { hasResource: (id) => Boolean(resources.getResource(id)), hasArea: (id) => Boolean(structure.getArea(id)) }); const annotationIds = new FactoryAnnotationIdGenerator(); const annotations = new FactoryAnnotationStore(annotationIds); const session = new ProjectSessionService(resources, operations, connections, structure, routes, annotations, workspaces, selection, resourceIds, operationIds, connectionIds, routeIds, annotationIds);
const context = { resources, operations, connections, structure, routes, annotations, project: session, selection }; const modelHistory = new CommandHistoryService(context, 200); session.attachHistory(modelHistory); const commands = new CommandFactory(modelHistory, context);
...
assert(commands.updateResource(resourceId, { name: 'Primary Machine' }) && commands.updateResource(resourceId, { worldX: 150, worldY: 250 }) && commands.updateResource(resourceId, { width: 240, depth: 120 }) && commands.updateResource(resourceId, { active: false }) && commands.updateResource(resourceId, { capacity: 2 }), 'Resource property commands execute');
...
const document = { format: 'ManufacturingFlowDesigner', schemaVersion: '1.4.0', applicationVersion: '0.6.0', project: { ...session.getMetadata(), name: 'Opened Project' }, resourceTemplates: RESOURCE_TEMPLATES, operationTemplates: OPERATION_TEMPLATES, resources: [], operations: [], connections: [], layoutBoundaries: [], walls: [], areas: [], aisles: [], factoryRoutes: [], factoryAnnotations: [], workspaces: { active: 'processFlow', processFlow: workspaces.getViewport('processFlow'), factoryLayout: workspaces.getViewport('factoryLayout') }, settings: session.getSettings() };
...
selection.setWorkspace('factoryLayout');
workspaces.activate('factoryLayout'); const boundary = commands.createBoundary([{ x: 0, y: 0 }, { x: 1000, y: 0 }, { x: 1000, y: 800 }, { x: 0, y: 800 }]); const wall = commands.createWall({ x: 500, y: 0 }, { x: 500, y: 800 }, 40); const area = commands.createArea(200, 200, 200, 100); const aisle = commands.createAisle([{ x: 100, y: 600 }, { x: 900, y: 600 }], 80); assert(boundary && wall && area && aisle, 'Factory structure create commands succeed'); const aisleId = aisle.id; modelHistory.undo(); assert(!structure.getAisle(aisleId), 'Aisle create Undo removes the entity'); modelHistory.redo(); assert(structure.getAisle(aisleId)?.id === aisleId, 'Aisle create Redo restores the stable ID'); const beforeThickness = structure.getWall(wall.id).thickness; assert(commands.updateWall(wall.id, { thickness: 75 }), 'Wall edit creates a command'); modelHistory.undo(); assert(structure.getWall(wall.id).thickness === beforeThickness, 'Wall edit Undo restores exact geometry'); modelHistory.redo(); assert(structure.getWall(wall.id).thickness === 75, 'Wall edit Redo restores the change'); assert(commands.deleteFactoryStructure('area', area.id) && !structure.getArea(area.id), 'Area delete command removes the entity'); modelHistory.undo(); assert(structure.getArea(area.id)?.id === area.id && selection.getSelection().id === area.id, 'Area delete Undo restores the entity and valid selection'); const countBeforeNoopStructure = modelHistory.getState().undoCount; assert(!commands.updateBoundary(boundary.id, { name: structure.getBoundary(boundary.id).name }) && modelHistory.getState().undoCount === countBeforeNoopStructure, 'Unchanged structure properties create no history entry');
