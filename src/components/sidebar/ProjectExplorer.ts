...
export function createProjectExplorer(operationsStore: OperationStore, connectionsStore: ConnectionStore, resourcesStore: ResourceStore, workspaceStore: WorkspaceStore, project: ProjectSessionService, selection: SelectionController, commands: CommandFactory): ProjectExplorerController {
  ...
  const render = (): void => {
    ...
    for (const resource of instances) { const count = operationsStore.getAssignmentCount(resource.id); const issues = resourceHealth.issues.filter((issue) => issue.resourceId === resource.id || issue.relatedResourceIds?.includes(resource.id)); const marker = issues.some((issue) => issue.severity === 'error') ? ' ⛔' : issues.length ? ' ⚠' : ''; const clearance = resource.clearance.enabled ? ` · ${resource.clearance.category} clearance` : ''; const entry = element('li'); const button = actionButton(`${resource.id} — ${resource.name} · ${resource.width}×${resource.depth} @ ${resource.rotationDegrees}°${clearance}${marker}`, 'process-operation resource-instance-entry'); button.title = `${resource.resourceType}; ${resource.active ? 'Active' : 'Inactive'}; ${count} assigned.${issues.length ? ` ${issues.map((issue) => issue.message).join(' ')}` : ''}`; button.setAttribute('aria-label', button.title); button.classList.toggle('process-operation--selected', resource.selected); button.addEventListener('click', (event) => { selectFromExplorer(event, { kind: 'resource', id: resource.id }); revealResource(resource.id); }); entry.append(button); resources.append(entry); }
