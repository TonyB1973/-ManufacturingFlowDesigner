...
export function createRightSidebar(resources: ResourceStore, operations: OperationStore, connections: ConnectionStore, workspaces: WorkspaceStore, selection: SelectionController, requestDelete: (resourceId: string) => void, project: ProjectSessionService): RightSidebarController {
  ...
  const renderResource = (): void => {
    ...
    const name = input('text'); name.value = resource.name; bindText(name, (value) => resources.updateResource(resource.id, { name: value }));
    const x = input('number'); x.value = String(resource.worldX); bindNumber(x, (value) => resources.updateResource(resource.id, { worldX: value }), Number.NEGATIVE_INFINITY);
    const y = input('number'); y.value = String(resource.worldY); bindNumber(y, (value) => resources.updateResource(resource.id, { worldY: value }), Number.NEGATIVE_INFINITY);
    const width = input('number', '5', MIN_RESOURCE_WIDTH); width.value = String(resource.width); bindNumber(width, (value) => resources.updateResource(resource.id, { width: value }), MIN_RESOURCE_WIDTH);
    const height = input('number', '5', MIN_RESOURCE_HEIGHT); height.value = String(resource.height); bindNumber(height, (value) => resources.updateResource(resource.id, { height: value }), MIN_RESOURCE_HEIGHT);
    const rotation = input('number', '5'); rotation.value = String(resource.rotationDegrees); bindNumber(rotation, (value) => resources.updateResource(resource.id, { rotationDegrees: value }), Number.NEGATIVE_INFINITY);
    const capacity = input('number', '1', 1); capacity.value = String(resource.capacity); bindNumber(capacity, (value) => resources.updateResource(resource.id, { capacity: value }), 1, true); x.disabled = resource.locked; y.disabled = resource.locked;
    const active = checkbox('Active'); active.checked = resource.active; active.addEventListener('change', () => resources.updateResource(resource.id, { active: active.checked })); const visible = checkbox('Visible'); visible.checked = resource.visible; visible.addEventListener('change', () => resources.updateResource(resource.id, { visible: visible.checked })); const locked = checkbox('Locked'); locked.checked = resource.locked; locked.addEventListener('change', () => resources.updateResource(resource.id, { locked: locked.checked })); const toggles = element('div', 'property-toggles'); toggles.append(toggle('Active', active), toggle('Visible', visible), toggle('Locked', locked));
    const help = element('p', 'property-help', 'For additional machinery capacity, normally add another physical resource instance instead of increasing capacity.'); const actions = element('div', 'resource-property-actions'); const duplicate = actionButton('Duplicate Resource', 'secondary-button'); duplicate.setAttribute('aria-label', 'Duplicate selected physical resource'); duplicate.addEventListener('click', () => { const copy = resources.duplicateResource(resource.id); reportStatus(copy ? `Resource duplicated: ${copy.id}` : 'Resource could not be duplicated'); }); const remove = actionButton('Delete Resource', 'secondary-button resource-delete-button'); remove.addEventListener('click', () => requestDelete(resource.id)); actions.append(duplicate, remove);
  ...
  const renderOperation = (): void => {
    ...
    const form = element('form', 'properties-form operation-properties'); form.addEventListener('submit', (event) => event.preventDefault()); const name = input('text'); name.value = operation.name; bindText(name, (value) => operations.updateOperation(operation.id, { name: value }));
    const type = element('select', 'property-input'); [...new Set(operations.getTemplates().map((template) => template.operationType))].forEach((value) => type.append(option(value))); type.value = operation.operationType; type.addEventListener('change', () => operations.updateOperation(operation.id, { operationType: type.value as OperationType }));
    const timing = element('select', 'property-input'); TIMING_CATEGORIES.forEach((value) => timing.append(option(value))); timing.value = operation.timingCategory; timing.addEventListener('change', () => operations.updateOperation(operation.id, { timingCategory: timing.value as OperationInstance['timingCategory'] }));
    const cycle = input('number', '0.001', 0.001); cycle.value = String(operation.cycleTimeSeconds); bindNumber(cycle, (value) => operations.updateOperation(operation.id, { cycleTimeSeconds: value }), 0.001); const sequence = input('number', '1', 1); sequence.value = String(operation.sequence); bindNumber(sequence, (value) => operations.updateOperation(operation.id, { sequence: value }), 1, true);
    const assignment = element('select', 'property-input'); assignment.append(option('Unassigned', '')); for (const resource of resources.getAssignableResources()) assignment.append(option(`${resource.id} — ${resource.name}`, resource.id)); assignment.value = operation.assignedResourceId && resources.getResource(operation.assignedResourceId)?.active ? operation.assignedResourceId : ''; assignment.addEventListener('change', () => { const resource = assignment.value ? resources.getResource(assignment.value) : undefined; if (assignment.value && !resource?.active) { reportStatus('Inactive resource cannot be assigned'); return; } operations.updateOperation(operation.id, { assignedResourceId: assignment.value || null }); reportStatus(assignment.value ? `Operation assigned to ${assignment.value}` : 'Operation unassigned'); });
    ...
    const notes = element('textarea', 'property-input property-textarea'); notes.value = operation.notes; notes.rows = 3; notes.addEventListener('change', () => operations.updateOperation(operation.id, { notes: notes.value }));
    const x = input('number'); x.value = String(operation.worldX); bindNumber(x, (value) => operations.updateOperation(operation.id, { worldX: value }), Number.NEGATIVE_INFINITY); const y = input('number'); y.value = String(operation.worldY); bindNumber(y, (value) => operations.updateOperation(operation.id, { worldY: value }), Number.NEGATIVE_INFINITY); const width = input('number', '5', MIN_OPERATION_WIDTH); width.value = String(operation.width); bindNumber(width, (value) => operations.updateOperation(operation.id, { width: value }), MIN_OPERATION_WIDTH); const height = input('number', '5', MIN_OPERATION_HEIGHT); height.value = String(operation.height); bindNumber(height, (value) => operations.updateOperation(operation.id, { height: value }), MIN_OPERATION_HEIGHT); x.disabled = operation.locked; y.disabled = operation.locked;
    const locked = checkbox('Locked'); locked.checked = operation.locked; locked.addEventListener('change', () => operations.updateOperation(operation.id, { locked: locked.checked })); const visible = checkbox('Visible'); visible.checked = operation.visible; visible.addEventListener('change', () => operations.updateOperation(operation.id, { visible: visible.checked })); const toggles = element('div', 'property-toggles'); toggles.append(toggle('Locked', locked), toggle('Visible', visible));
  ...
  const renderConnection = (connection: ProcessConnection): void => {
    ...
    const type = element('select', 'property-input'); (['Standard', 'Rework', 'Alternate', 'Information'] as const).forEach((value) => type.append(option(value))); type.value = connection.connectionType; type.addEventListener('change', () => reportStatus(connections.updateConnection(connection.id, { connectionType: type.value as ConnectionType }) ? 'Connection type updated' : 'Duplicate Standard connection not permitted'));
    const label = input('text'); label.maxLength = 120; label.value = connection.label; bindText(label, (value) => connections.updateConnection(connection.id, { label: value }));
    const locked = checkbox('Locked'); locked.checked = connection.locked; locked.addEventListener('change', () => { connections.updateConnection(connection.id, { locked: locked.checked }); reportStatus(`Connection ${locked.checked ? 'locked' : 'unlocked'}`); }); const visible = checkbox('Visible'); visible.checked = connection.visible; visible.addEventListener('change', () => { connections.updateConnection(connection.id, { visible: visible.checked }); reportStatus(`Connection ${visible.checked ? 'shown' : 'hidden'}`); }); const toggles = element('div', 'property-toggles'); toggles.append(toggle('Locked', locked), toggle('Visible', visible));
    const actions = element('div', 'connection-property-actions'); const reverse = actionButton('Reverse Direction', 'secondary-button'); const remove = actionButton('Delete Connection', 'secondary-button connection-delete-button'); reverse.addEventListener('click', () => { const result = connections.reverseConnection(connection.id); reportStatus(result === 'updated' ? 'Connection reversed' : result === 'duplicate' ? 'Cannot reverse: duplicate Standard connection' : result === 'locked' ? 'Connection is locked' : 'Connection not found'); }); remove.addEventListener('click', () => { const result = connections.deleteConnection(connection.id); reportStatus(result === 'deleted' ? 'Connection deleted' : result === 'locked' ? 'Connection is locked' : 'Connection not found'); }); actions.append(reverse, remove);
