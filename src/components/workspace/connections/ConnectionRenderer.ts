...
export class ConnectionRenderer {
  ...
  private render(connection: ProcessConnection): void {
    ...
    node.group.classList.toggle('process-connection--selected', connection.selected); node.group.classList.toggle('process-connection--primary', this.store.getSelectedConnection()?.id === connection.id); node.group.classList.toggle('process-connection--warning', connection.routeStatus === 'fallback'); node.group.classList.toggle('process-connection--locked', connection.locked); node.group.classList.toggle('process-connection--delete-target', false); node.group.dataset.connectionType = connection.connectionType;
