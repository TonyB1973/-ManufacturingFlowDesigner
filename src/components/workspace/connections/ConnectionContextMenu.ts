...
export interface ConnectionContextActions { readonly onDelete: () => void; readonly onReverse: () => void; readonly onSelectSource: () => void; readonly onSelectTarget: () => void; }
...
export function createConnectionContextMenu(viewport: HTMLElement): ConnectionContextMenuController {
  ...
  return {
    open: (x, y, connectionId, actions) => {
      ...
      add('Delete Connection', actions.onDelete); add('Reverse Direction', actions.onReverse); add('Select Source Operation', actions.onSelectSource); add('Select Target Operation', actions.onSelectTarget);
