...
import type { CanvasWorkspaceId, WorkspaceId } from '../../../models/workspace/Workspace';
...
import { requestAddOperationToStandardWork, requestShowOperationInStandardWork } from '../../../core/events/standardWorkEvents';
...
export function createCanvasViewport(application: HTMLElement, resourceStore: ResourceStore, operationStore: OperationStore, connectionStore: ConnectionStore, structureStore: FactoryStructureStore, routeStore: FactoryRouteStore, annotationStore: FactoryAnnotationStore, annotationResolver: AnnotationAnchorResolver, projectSession: ProjectSessionService, workspaceStore: WorkspaceStore, selectionStore: SelectionController, commands: CommandFactory, routeCommands: FactoryRouteCommandFactory, annotationCommands: FactoryAnnotationCommandFactory, editing: ApplicationClipboardService, geometrySelection: GeometrySelectionService, geometryEditing: GeometryEditingService, geometryCommands: GeometryCommandFactory, standardWorkPanel: HTMLElement, availabilityPanel: HTMLElement, scenarioPanel: HTMLElement, callbacks: CanvasViewportCallbacks): CanvasViewportController {
  ...
  const processTab = element('button', 'workspace-tab', 'Process Flow'); const layoutTab = element('button', 'workspace-tab', 'Factory Layout'); const standardWorkTab = element('button', 'workspace-tab', 'Standard Work'); const availabilityTab = element('button', 'workspace-tab', 'Availability'); const scenariosTab = element('button', 'workspace-tab', 'Scenarios'); for (const tab of [processTab, layoutTab, standardWorkTab, availabilityTab, scenariosTab]) tab.setAttribute('role', 'tab'); workspaceTabs.append(processTab, layoutTab, standardWorkTab, availabilityTab, scenariosTab); workspaceHeader.append(workspaceTabs, canvasTitle);
  ...
  let activeCanvasWorkspace: CanvasWorkspaceId = activeWorkspace === 'factoryLayout' ? 'factoryLayout' : 'processFlow';
  ...
  const saveViewport = (): void => workspaceStore.updateViewport(activeCanvasWorkspace, { panX: state.panX, panY: state.panY, zoom: state.zoom, gridVisible: state.gridVisible, originVisible: state.originVisible, snapEnabled: snap.enabled });
  const loadViewport = (workspaceId: CanvasWorkspaceId): void => { const stored = workspaceStore.getViewport(workspaceId); Object.assign(state, { panX: stored.panX, panY: stored.panY, zoom: stored.zoom, gridVisible: stored.gridVisible, originVisible: stored.originVisible, tool: 'select' }); snap.enabled = stored.snapEnabled; };
  ...
  workspace.append(workspaceHeader, toolbar.element, viewport, standardWorkPanel, availabilityPanel, scenarioPanel);
  ...
  const handleObjectKeyDown = (event: KeyboardEvent): void => {
    if (activeWorkspace !== 'processFlow' && activeWorkspace !== 'factoryLayout') return;
  ...
  processTab.addEventListener('click', () => activateWorkspace('processFlow')); layoutTab.addEventListener('click', () => activateWorkspace('factoryLayout')); standardWorkTab.addEventListener('click', () => activateWorkspace('standardWork')); availabilityTab.addEventListener('click', () => activateWorkspace('availability')); scenariosTab.addEventListener('click', () => activateWorkspace('scenarios'));
  ...
  const renderWorkspace = (workspaceId: WorkspaceId): void => {
    ...
    activeWorkspace = workspaceId; const standardActive = workspaceId === 'standardWork'; const availabilityActive = workspaceId === 'availability'; const scenariosActive = workspaceId === 'scenarios'; const canvasActive = workspaceId === 'processFlow' || workspaceId === 'factoryLayout'; if (canvasActive) { activeCanvasWorkspace = workspaceId; loadViewport(workspaceId); selectionStore.setWorkspace(workspaceId); grid.setWorkspace(workspaceId); } else selectionStore.clear(); connectionInteraction?.toolChanged();
    const processActive = workspaceId === 'processFlow'; const layoutActive = workspaceId === 'factoryLayout'; toolbar.element.hidden = !canvasActive; viewport.hidden = !canvasActive; standardWorkPanel.hidden = !standardActive; availabilityPanel.hidden = !availabilityActive; scenarioPanel.hidden = !scenariosActive; toolbar.setConnectionToolsEnabled(processActive); toolbar.setFactoryToolsEnabled(layoutActive); processTab.setAttribute('aria-selected', String(processActive)); layoutTab.setAttribute('aria-selected', String(layoutActive)); standardWorkTab.setAttribute('aria-selected', String(standardActive)); availabilityTab.setAttribute('aria-selected', String(availabilityActive)); scenariosTab.setAttribute('aria-selected', String(scenariosActive)); processTab.tabIndex = processActive ? 0 : -1; layoutTab.tabIndex = layoutActive ? 0 : -1; standardWorkTab.tabIndex = standardActive ? 0 : -1; availabilityTab.tabIndex = availabilityActive ? 0 : -1; scenariosTab.tabIndex = scenariosActive ? 0 : -1; canvasTitle.textContent = processActive ? 'Process Flow — Operations' : layoutActive ? 'Factory Layout — Resources, Structure & Annotations' : standardActive ? 'Standard Work — Combination Chart' : availabilityActive ? 'Availability — Shifts & Calendars' : 'Scenarios — Baseline Comparison'; if (canvasActive) viewport.setAttribute('aria-label', `${processActive ? 'Process Flow' : 'Factory Layout'} engineering canvas. Use the mouse wheel to zoom and middle mouse or Space plus drag to pan.`); callbacks.onWorkspaceChange(workspaceId); callbacks.onStatusChange(`Workspace: ${processActive ? 'Process Flow' : layoutActive ? 'Factory Layout' : standardActive ? 'Standard Work' : availabilityActive ? 'Availability' : 'Scenarios'}`); if (canvasActive) requestRender();
  };
  const unsubscribeWorkspace = workspaceStore.subscribe(renderWorkspace); grid.setWorkspace(activeCanvasWorkspace); renderWorkspace(activeWorkspace);
  requestRender();
