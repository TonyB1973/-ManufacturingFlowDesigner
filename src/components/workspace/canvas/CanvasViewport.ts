...
export function createCanvasViewport(application: HTMLElement, resourceStore: ResourceStore, operationStore: OperationStore, connectionStore: ConnectionStore, structureStore: FactoryStructureStore, routeStore: FactoryRouteStore, annotationStore: FactoryAnnotationStore, annotationResolver: AnnotationAnchorResolver, projectSession: ProjectSessionService, workspaceStore: WorkspaceStore, selectionStore: SelectionController, commands: CommandFactory, routeCommands: FactoryRouteCommandFactory, annotationCommands: FactoryAnnotationCommandFactory, editing: ApplicationClipboardService, geometrySelection: GeometrySelectionService, geometryEditing: GeometryEditingService, geometryCommands: GeometryCommandFactory, standardWorkPanel: HTMLElement, availabilityPanel: HTMLElement, callbacks: CanvasViewportCallbacks): CanvasViewportController {
  ...
  const processTab = element('button', 'workspace-tab', 'Process Flow'); const layoutTab = element('button', 'workspace-tab', 'Factory Layout'); const standardWorkTab = element('button', 'workspace-tab', 'Standard Work'); const availabilityTab = element('button', 'workspace-tab', 'Availability'); for (const tab of [processTab, layoutTab, standardWorkTab, availabilityTab]) tab.setAttribute('role', 'tab'); workspaceTabs.append(processTab, layoutTab, standardWorkTab, availabilityTab); workspaceHeader.append(workspaceTabs, canvasTitle);
  ...
  workspace.append(workspaceHeader, toolbar.element, viewport, standardWorkPanel, availabilityPanel);
  ...
  const handleObjectKeyDown = (event: KeyboardEvent): void => {
    if (activeWorkspace === 'standardWork') return;
  ...
  processTab.addEventListener('click', () => activateWorkspace('processFlow')); layoutTab.addEventListener('click', () => activateWorkspace('factoryLayout')); standardWorkTab.addEventListener('click', () => activateWorkspace('standardWork')); availabilityTab.addEventListener('click', () => activateWorkspace('availability'));
  ...
  const renderWorkspace = (workspaceId: WorkspaceId): void => {
    ...
    activeWorkspace = workspaceId; const standardActive = workspaceId === 'standardWork'; const availabilityActive = workspaceId === 'availability'; const canvasActive = workspaceId === 'processFlow' || workspaceId === 'factoryLayout'; if (canvasActive) { activeCanvasWorkspace = workspaceId; loadViewport(workspaceId); selectionStore.setWorkspace(workspaceId); grid.setWorkspace(workspaceId); } else selectionStore.clear(); connectionInteraction?.toolChanged();
    const processActive = workspaceId === 'processFlow'; const layoutActive = workspaceId === 'factoryLayout'; toolbar.element.hidden = !canvasActive; viewport.hidden = !canvasActive; standardWorkPanel.hidden = !standardActive; availabilityPanel.hidden = !availabilityActive; toolbar.setConnectionToolsEnabled(processActive); toolbar.setFactoryToolsEnabled(layoutActive); processTab.setAttribute('aria-selected', String(processActive)); layoutTab.setAttribute('aria-selected', String(layoutActive)); standardWorkTab.setAttribute('aria-selected', String(standardActive)); availabilityTab.setAttribute('aria-selected', String(availabilityActive)); processTab.tabIndex = processActive ? 0 : -1; layoutTab.tabIndex = layoutActive ? 0 : -1; standardWorkTab.tabIndex = standardActive ? 0 : -1; availabilityTab.tabIndex = availabilityActive ? 0 : -1; canvasTitle.textContent = processActive ? 'Process Flow — Operations' : layoutActive ? 'Factory Layout — Resources, Structure & Annotations' : standardActive ? 'Standard Work — Combination Chart' : 'Availability — Shifts & Calendars'; if (canvasActive) viewport.setAttribute('aria-label', `${processActive ? 'Process Flow' : 'Factory Layout'} engineering canvas. Use the mouse wheel to zoom and middle mouse or Space plus drag to pan.`); callbacks.onWorkspaceChange(workspaceId); callbacks.onStatusChange(`Workspace: ${processActive ? 'Process Flow' : layoutActive ? 'Factory Layout' : standardActive ? 'Standard Work' : 'Availability'}`); if (canvasActive) requestRender();
  };
