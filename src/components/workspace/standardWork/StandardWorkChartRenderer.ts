...
export function renderStandardWorkChart(options: StandardWorkChartRenderOptions): StandardWorkChartRenderResult {
  ...
  const timeExtent = settings.includeTaktInFit && options.taktTimeSeconds !== null && options.taktTimeSeconds !== undefined ? Math.max(schedule.chartCycleSpanSeconds, options.taktTimeSeconds) : schedule.chartCycleSpanSeconds;
  const width = Math.max(options.viewportWidth, padding * 2 + Math.max(1, timeExtent) * options.pixelsPerSecond);
  ...
  if (settings.showTaktLine && settings.shadeBeyondTakt && options.taktTimeSeconds !== null && options.taktTimeSeconds !== undefined && Number.isFinite(options.taktTimeSeconds) && options.taktTimeSeconds > 0 && schedule.chartCycleSpanSeconds > options.taktTimeSeconds) { const x = padding + options.taktTimeSeconds * options.pixelsPerSecond; const shade = svg('rect'); shade.setAttribute('x', String(x)); shade.setAttribute('y', String(axisHeight)); shade.setAttribute('width', String(Math.max(0, width - x))); shade.setAttribute('height', String(Math.max(0, height - axisHeight))); shade.classList.add('standard-work-beyond-takt'); drawing.append(shade); }
  ...
  if (settings.showTaktLine && options.taktTimeSeconds !== null && options.taktTimeSeconds !== undefined && Number.isFinite(options.taktTimeSeconds) && options.taktTimeSeconds > 0) {
    const x = padding + options.taktTimeSeconds * options.pixelsPerSecond;
    const line = svg('line'); line.setAttribute('x1', String(x)); line.setAttribute('x2', String(x)); line.setAttribute('y1', String(axisHeight)); line.setAttribute('y2', String(height)); line.classList.add('standard-work-takt-line'); line.setAttribute('role', 'img'); line.setAttribute('aria-label', `Takt time ${formatDuration(options.taktTimeSeconds, options.timeFormat)}`); const title = svg('title'); title.textContent = `Takt time ${formatDuration(options.taktTimeSeconds, options.timeFormat)}`; line.append(title); drawing.append(line);
    if (settings.showTaktValue) { const text = svg('text'); text.setAttribute('x', String(x + 5)); text.setAttribute('y', '34'); text.classList.add('standard-work-takt-label'); text.textContent = `TAKT ${formatDuration(options.taktTimeSeconds, options.timeFormat)}`; drawing.append(text); }
  }
