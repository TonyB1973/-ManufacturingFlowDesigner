...
export const CLIPBOARD_LIMITS = { resources: 5_000, operations: 10_000, connections: 20_000, structures: 20_000, routes: 20_000 } as const;
...
export class ApplicationClipboardService {
  ...
  public copy(): EditingResult {
    ...
    this.clipboard = candidate; const count = candidate.resources.length + candidate.operations.length + candidate.connections.length + candidate.walls.length + candidate.areas.length + candidate.aisles.length + candidate.routes.length;
  ...
  private capture(skipLocked = false): ApplicationClipboard | { readonly message: string } {
    ...
    if (!resources.length && !operations.length && !connections.length && !walls.length && !areas.length && !aisles.length && !routes.length) return { message: items.some((item) => item.kind === 'boundary') ? 'Factory boundary cannot be copied' : skipLocked ? 'No unlocked items selected' : 'Nothing selected to copy' };
    if (resources.length > CLIPBOARD_LIMITS.resources || operations.length > CLIPBOARD_LIMITS.operations || connections.length > CLIPBOARD_LIMITS.connections || walls.length + areas.length + aisles.length > CLIPBOARD_LIMITS.structures || routes.length > CLIPBOARD_LIMITS.routes) return { message: 'Selection exceeds the application clipboard safety limit' };
    const boxes = [
    ...
    return { sourceWorkspace: workspace, resources: resources.map((value) => ({ sourceId: value.id, value: cloneResource(value) })), operations: operations.map((value) => ({ sourceId: value.id, value: cloneOperation(value) })), connections: connections.map((value) => ({ sourceId: value.id, value: { ...cloneConnection(value), routePoints: [], routeStatus: 'clear' } })), walls: walls.map((value) => ({ sourceId: value.id, value: cloneWall(value) })), areas: areas.map((value) => ({ sourceId: value.id, value: cloneArea(value) })), aisles: aisles.map((value) => ({ sourceId: value.id, value: cloneAisle(value) })), routes: routes.map((value) => ({ sourceId: value.id, value: cloneFactoryRoute(value) })), bounds, copiedAt: Date.now(), pasteCount: 0 };
  }
  ...
  private pasteClipboard(source: ApplicationClipboard, updateClipboard: boolean, description: string): EditingResult {
    ...
    if (source.sourceWorkspace === 'factoryLayout') {
      const resourceMap = new Map<string, string>(); const areaMap = new Map<string, string>(); const existingNames = new Set(this.resources.getPlacedResources().map((item) => item.name));
      ...
      const routeSnapshots = source.routes.map(({ value }) => ({ ...cloneFactoryRoute(value), id: this.routeIds.next(), name: `${value.name} copy`, source: remapEndpoint(value.source), target: remapEndpoint(value.target), waypoints: value.waypoints.map((point) => ({ x: offset(point.x), y: offset(point.y) })), locked: false }));
      const count = resourceSnapshots.length + walls.length + areas.length + aisles.length + routeSnapshots.length; if (!count) return { ok: false, message: 'Factory selection is empty', count: 0 };
      if (!this.commands.insertFactorySelection({ resources: resourceSnapshots, walls, areas, aisles, routes: routeSnapshots }, description)) return { ok: false, message: 'Factory selection could not be pasted', count: 0 }; if (updateClipboard) this.clipboard = { ...source, pasteCount }; return { ok: true, message: `${description.replace(' selection', '')}d ${count} items`, count };
    }
