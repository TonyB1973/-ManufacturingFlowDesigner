...
export class StandardWorkStore {
  ...
  private operatorStudy: (id: string) => string | null = () => null;
  ...
  public setOperatorResolver(resolver: (id: string) => string | null): void { this.operatorStudy = resolver; }
  ...
  public replaceAll(studies: readonly StandardWorkStudy[], entries: readonly StandardWorkEntry[], notify = true): void { this.studies.clear(); this.entries.clear(); studies.forEach((study) => this.studies.set(study.id, cloneStandardWorkStudy(study))); entries.forEach((entry) => this.entries.set(entry.id, cloneStandardWorkEntry(entry))); const studyIds = studies.map((item) => item.id); const entryIds = entries.map((item) => item.id); if (this.studyIds.reset) this.studyIds.reset(studyIds); else this.studyIds.ensureAfter(studyIds); if (this.entryIds.reset) this.entryIds.reset(entryIds); else this.entryIds.ensureAfter(entryIds); if (notify) this.publishReset(); }
