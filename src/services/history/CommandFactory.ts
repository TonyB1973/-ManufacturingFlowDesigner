...
export class CommandFactory {
  ...
  public deleteFactoryStructure(kind: 'boundary' | 'wall' | 'area' | 'aisle', id: string): boolean { const value = kind === 'boundary' ? this.context.structure.getBoundary(id) : kind === 'wall' ? this.context.structure.getWall(id) : kind === 'area' ? this.context.structure.getArea(id) : this.context.structure.getAisle(id); if (!value || value.locked) return false; const snapshot = kind === 'boundary' ? cloneBoundary(value as FactoryLayoutBoundary) : kind === 'wall' ? cloneWall(value as FactoryWall) : kind === 'area' ? cloneArea(value as FactoryArea) : cloneAisle(value as FactoryAisle); return this.run(new ReversibleCommand(`Delete ${kind} ${id}`, [id], 'factoryLayout', ({ structure, selection }) => { if (!structure.delete(kind, id)) throw new Error(`${kind} could not be deleted.`); selection.remove({ kind, id }); }, ({ structure, selection }) => { const restored = kind === 'boundary' ? structure.restoreBoundary(snapshot as FactoryLayoutBoundary) : kind === 'wall' ? structure.restoreWall(snapshot as FactoryWall) : kind === 'area' ? structure.restoreArea(snapshot as FactoryArea) : structure.restoreAisle(snapshot as FactoryAisle); if (!restored) throw new Error(`${kind} could not be restored.`); selection.select({ kind, id }); })); }
