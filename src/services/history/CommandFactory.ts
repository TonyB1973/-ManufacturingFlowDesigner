...
export class CommandFactory {
  ...
  public deleteFactoryStructure(kind: 'boundary' | 'wall' | 'area' | 'aisle', id: string): boolean {
    ...
    return this.run(new ReversibleCommand(`Delete ${kind} ${id}`, [id, ...attachedRoutes.map((route) => route.id)], 'factoryLayout',
      ({ structure, routes, selection }) => { if (attachedRoutes.length) routes.deleteAttachedToArea(id); if (!structure.delete(kind, id)) throw new Error(`${kind} could not be deleted.`); selection.remove({ kind, id }); },
      ({ structure, routes, selection }) => { const restored = kind === 'boundary' ? structure.restoreBoundary(snapshot as FactoryLayoutBoundary) : kind === 'wall' ? structure.restoreWall(snapshot as FactoryWall) : kind === 'area' ? structure.restoreArea(snapshot as FactoryArea) : structure.restoreAisle(snapshot as FactoryAisle); if (!restored) throw new Error(`${kind} could not be restored.`); for (const route of attachedRoutes) if (!routes.restoreRoute(route)) throw new Error(`FactoryRoute ${route.id} could not be restored.`); selection.select({ kind, id }); }));
  }
  ...
  public deleteResource(resourceId: string): DeleteResult {
    ...
    const snapshot = cloneResource(resource); const assignments = this.context.operations.getOperations().filter((operation) => operation.assignedResourceId === resourceId).map((operation) => operation.id); const attachedRoutes = this.context.routes.getRoutesForResource(resourceId).map(cloneFactoryRoute);
    const command = new ReversibleCommand(`Delete resource ${resourceId}`, [resourceId, ...assignments, ...attachedRoutes.map((route) => route.id)], 'factoryLayout',
      ({ resources, operations, routes }) => { routes.deleteAttachedToResource(resourceId); if (resources.deleteResource(resourceId) !== 'deleted') throw new Error('Resource could not be deleted.'); operations.unassignResource(resourceId); },
      ({ resources, operations, routes }) => { if (!resources.restoreResource(snapshot)) throw new Error('Resource could not be restored.'); for (const operationId of assignments) if (!operations.updateOperation(operationId, { assignedResourceId: resourceId })) throw new Error(`Assignment for ${operationId} could not be restored.`); for (const route of attachedRoutes) if (!routes.restoreRoute(route)) throw new Error(`FactoryRoute ${route.id} could not be restored.`); resources.selectResource(resourceId); });
  ...
  public insertFactorySelection(snapshots: { readonly resources: readonly PlacedResource[]; readonly walls: readonly FactoryWall[]; readonly areas: readonly FactoryArea[]; readonly aisles: readonly FactoryAisle[]; readonly routes: readonly FactoryRoute[] }, description = 'Paste factory selection'): boolean {
    const resources = snapshots.resources.map(cloneResource); const walls = snapshots.walls.map(cloneWall); const areas = snapshots.areas.map(cloneArea); const aisles = snapshots.aisles.map(cloneAisle); const routes = snapshots.routes.map(cloneFactoryRoute);
    const refs: SelectionItem[] = [...resources.map((item) => ({ kind: 'resource' as const, id: item.id })), ...walls.map((item) => ({ kind: 'wall' as const, id: item.id })), ...areas.map((item) => ({ kind: 'area' as const, id: item.id })), ...aisles.map((item) => ({ kind: 'aisle' as const, id: item.id })), ...routes.map((item) => ({ kind: 'factoryRoute' as const, id: item.id }))];
    if (!refs.length) return false;
    return this.run(new ReversibleCommand(description, refs.map((item) => item.id), 'factoryLayout',
  ...
  public deleteResources(resourceIds: readonly string[], description = 'Delete resources'): DeleteResult {
    ...
    const assignments = this.context.operations.getOperations().filter((operation) => snapshots.some((resource) => resource.id === operation.assignedResourceId)).map((operation) => ({ id: operation.id, resourceId: operation.assignedResourceId! })); const attachedRoutes = snapshots.flatMap((resource) => this.context.routes.getRoutesForResource(resource.id)).filter((route, index, all) => all.findIndex((candidate) => candidate.id === route.id) === index).map(cloneFactoryRoute);
    const command = new ReversibleCommand(description, [...snapshots.map((item) => item.id), ...assignments.map((item) => item.id), ...attachedRoutes.map((route) => route.id)], 'factoryLayout',
      ({ resources, operations, routes }) => { for (const item of snapshots) { routes.deleteAttachedToResource(item.id); if (resources.deleteResource(item.id) !== 'deleted') throw new Error(`Resource ${item.id} could not be deleted.`); operations.unassignResource(item.id); } },
      ({ resources, operations, routes, selection }) => { for (const item of snapshots) if (!resources.restoreResource(item)) throw new Error(`Resource ${item.id} could not be restored.`); for (const item of assignments) if (!operations.updateOperation(item.id, { assignedResourceId: item.resourceId })) throw new Error(`Assignment ${item.id} could not be restored.`); for (const route of attachedRoutes) if (!routes.restoreRoute(route)) throw new Error(`FactoryRoute ${route.id} could not be restored.`); const refs: SelectionItem[] = snapshots.map((item) => ({ kind: 'resource', id: item.id })); selection.set(refs, refs.at(-1)); });
    return this.run(command) ? 'deleted' : 'none';
  }
