...
import { cloneStandardWorkPlanning, createDefaultStandardWorkPlanning } from '../../models/standardWork/StandardWorkPlanning';
...
export class StandardWorkCommandFactory {
  ...
  public createStudy(name?: string): StandardWorkStudy | null {
    ...
    const command = new ReversibleCommand(`Create Standard Work study ${id}`, [id, operator.id], 'standardWork',
      ...
      ({ standardWork, standardWorkOperators, standardWorkPlanning, standardWorkSelection }) => { standardWorkPlanning.delete(id); standardWorkOperators.deleteStudy(id); if (!standardWork.deleteStudy(id)) throw new Error('Study creation could not be undone.'); standardWorkSelection.clear(); });
  ...
  public duplicateStudy(id: string): StandardWorkStudy | null {
    ...
    const planning = { ...(this.context.standardWorkPlanning.get(id) ?? createDefaultStandardWorkPlanning(id)), studyId: study.id };
    const command = new ReversibleCommand(`Duplicate Standard Work study ${id} as ${study.id}`, [study.id, ...operators.map((item) => item.id), ...entries.map((entry) => entry.id), ...handovers.map((item) => item.id)], 'standardWork',
      ({ standardWork, standardWorkOperators, standardWorkHandovers, standardWorkPlanning, standardWorkSelection }) => { if (!standardWork.restoreStudy(study) || !standardWorkPlanning.restore(planning)) throw new Error('Duplicated study could not be restored.'); for (const operator of operators) if (!standardWorkOperators.restoreOperator(operator)) throw new Error('Duplicated operator could not be restored.'); for (const entry of entries) if (!standardWork.restoreEntry(entry)) throw new Error('Duplicated study entry could not be restored.'); for (const handover of handovers) if (!standardWorkHandovers.restoreHandover(handover)) throw new Error('Duplicated handover could not be restored.'); standardWorkSelection.select({ kind: 'standardWorkStudy', id: study.id }); },
      ({ standardWork, standardWorkOperators, standardWorkHandovers, standardWorkPlanning, standardWorkSelection }) => { standardWorkPlanning.delete(study.id); standardWorkHandovers.deleteStudy(study.id); standardWorkOperators.deleteStudy(study.id); if (!standardWork.deleteStudy(study.id)) throw new Error('Study duplication could not be undone.'); standardWorkSelection.select({ kind: 'standardWorkStudy', id }); });
    return this.run(command) ? cloneStandardWorkStudy(study) : null;
  }

  public deleteStudy(id: string): boolean {
    const source = this.context.standardWork.getStudy(id); if (!source) return false; const study = cloneStandardWorkStudy(source); const planning = cloneStandardWorkPlanning(this.context.standardWorkPlanning.get(id) ?? createDefaultStandardWorkPlanning(id)); const operators = this.context.standardWorkOperators.getOperators(id).map(cloneStandardWorkOperator); const entries = this.context.standardWork.getEntries(id).map(cloneStandardWorkEntry); const handovers = this.context.standardWorkHandovers.getHandovers(id).map(cloneStandardWorkHandover);
