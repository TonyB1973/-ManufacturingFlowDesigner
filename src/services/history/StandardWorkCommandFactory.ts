...
export class StandardWorkCommandFactory {
  ...
  public createStudy(name?: string): StandardWorkStudy | null {
    ...
    const command = new ReversibleCommand(`Create Standard Work study ${id}`, [id], 'standardWork',
      ({ standardWork, standardWorkSelection }) => { if (!standardWork.restoreStudy(study)) throw new Error('Study could not be created.'); standardWorkSelection.select({ kind: 'standardWorkStudy', id }); },
      ({ standardWork, standardWorkSelection }) => { if (!standardWork.deleteStudy(id)) throw new Error('Study creation could not be undone.'); standardWorkSelection.clear(); });
  ...
  public duplicateStudy(id: string): StandardWorkStudy | null {
    ...
    const entries = this.context.standardWork.getEntries(id).map((entry) => ({ ...entry, id: this.context.standardWork.nextEntryId(), studyId: study.id }));
    const command = new ReversibleCommand(`Duplicate Standard Work study ${id} as ${study.id}`, [study.id, ...entries.map((entry) => entry.id)], 'standardWork',
      ({ standardWork, standardWorkSelection }) => { if (!standardWork.restoreStudy(study)) throw new Error('Duplicated study could not be restored.'); for (const entry of entries) if (!standardWork.restoreEntry(entry)) throw new Error('Duplicated study entry could not be restored.'); standardWorkSelection.select({ kind: 'standardWorkStudy', id: study.id }); },
      ({ standardWork, standardWorkSelection }) => { if (!standardWork.deleteStudy(study.id)) throw new Error('Study duplication could not be undone.'); standardWorkSelection.select({ kind: 'standardWorkStudy', id }); });
    return this.run(command) ? cloneStandardWorkStudy(study) : null;
  }

  public deleteStudy(id: string): boolean {
    const source = this.context.standardWork.getStudy(id); if (!source) return false; const study = cloneStandardWorkStudy(source); const entries = this.context.standardWork.getEntries(id).map(cloneStandardWorkEntry);
    return this.run(new ReversibleCommand(`Delete Standard Work study ${id}`, [id, ...entries.map((entry) => entry.id)], 'standardWork',
      ({ standardWork, standardWorkSelection }) => { if (!standardWork.deleteStudy(id)) throw new Error('Study could not be deleted.'); standardWorkSelection.clear(); },
      ({ standardWork, standardWorkSelection }) => { if (!standardWork.restoreStudy(study)) throw new Error('Study could not be restored.'); for (const entry of entries) if (!standardWork.restoreEntry(entry)) throw new Error('Study entry could not be restored.'); standardWorkSelection.select({ kind: 'standardWorkStudy', id }); }));
  }
  ...
  public addOperation(studyId: string, operationId: string): AddStandardWorkEntryResult {
    ...
    if (!this.context.standardWork.getStudy(studyId) || !this.context.operations.getOperation(operationId)) return { kind: 'invalid' };
    const order = (this.context.standardWork.getEntries(studyId).at(-1)?.order ?? 0) + 10;
    const entry: StandardWorkEntry = { id: this.context.standardWork.nextEntryId(), studyId, operationId, order, occurrences: 1, enabled: true, notes: '' };
  ...
  public populate(studyId: string): readonly StandardWorkEntry[] {
    if (!this.context.standardWork.getStudy(studyId)) return [];
    ...
    const entries = this.context.operations.sortedOperations().filter((operation) => !existing.has(operation.id)).map((operation) => ({ id: this.context.standardWork.nextEntryId(), studyId, operationId: operation.id, order: order += 10, occurrences: 1, enabled: true, notes: '' } satisfies StandardWorkEntry));
  ...
  public removeEntry(id: string): boolean {
    const source = this.context.standardWork.getEntry(id); if (!source) return false; const entry = cloneStandardWorkEntry(source);
    return this.run(new ReversibleCommand(`Remove ${entry.operationId} from ${entry.studyId}`, [id, entry.studyId, entry.operationId], 'standardWork',
      ({ standardWork, standardWorkSelection }) => { if (!standardWork.deleteEntry(id)) throw new Error('Entry could not be removed.'); standardWorkSelection.select({ kind: 'standardWorkStudy', id: entry.studyId }); },
      ({ standardWork, standardWorkSelection }) => { if (!standardWork.restoreEntry(entry)) throw new Error('Entry could not be restored.'); standardWorkSelection.select({ kind: 'standardWorkEntry', id }); }));
  }
