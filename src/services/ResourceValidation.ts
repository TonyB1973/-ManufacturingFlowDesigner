...
export interface ResourceValidationIssue { readonly resourceId: string; readonly severity: 'error' | 'warning'; readonly code: string; readonly message: string; }
...
export function validateResources(resources: readonly ResourceInstance[], templates: readonly ResourceTemplate[], assignmentCount: (id: string) => number): ResourceValidationSummary {
  ...
  for (const resource of resources) {
    ...
    if (!Number.isFinite(resource.width) || !Number.isFinite(resource.height) || resource.width <= 0 || resource.height <= 0) issues.push(issue(resource.id, 'error', 'invalid-dimensions', 'Resource dimensions are invalid.'));
