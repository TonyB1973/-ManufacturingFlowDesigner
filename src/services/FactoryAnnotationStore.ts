...
export class FactoryAnnotationStore {
  ...
  public replaceAll(values: readonly FactoryAnnotation[], notify = true): void { this.annotations.clear(); for (const value of values) this.annotations.set(value.id, cloneFactoryAnnotation(value)); const ids = values.map((value) => value.id); if (this.ids.reset) this.ids.reset(ids); else this.ids.ensureAfter(ids); if (notify) this.publishReset(); }
