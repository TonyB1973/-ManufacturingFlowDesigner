...
import { StandardWorkPlanningCommandFactory } from '../services/history/StandardWorkPlanningCommandFactory';
...
import { calculateStandardWorkTaktWithAvailability } from '../services/standardWork/StandardWorkTaktService';
...
import { AvailabilitySelectionStore } from '../services/availability/AvailabilitySelectionStore';
import { AvailabilityCommandFactory } from '../services/history/AvailabilityCommandFactory';
import { createAvailabilityWorkspace } from '../components/workspace/availability/AvailabilityWorkspace';
import { validateAvailability } from '../services/availability/AvailabilityValidationService';
import { ScenarioSelectionStore } from '../services/scenarios/ScenarioSelectionStore';
import { ScenarioCommandFactory } from '../services/history/ScenarioCommandFactory';
import { createScenarioWorkspace } from '../components/workspace/scenarios/ScenarioWorkspace';
...
export function createAppShell(): AppShellResult {
  ...
  const standardWorkSelection = new StandardWorkSelectionStore(); const availabilitySelection = new AvailabilitySelectionStore(); const scenarioSelection = new ScenarioSelectionStore(); const standardWorkStore = new StandardWorkStore(new StandardWorkStudyIdGenerator(), new StandardWorkEntryIdGenerator(), (id) => Boolean(operationStore.getOperation(id)));
  ...
  projectSession = new ProjectSessionService(resourceStore, operationStore, connectionStore, structureStore, routeStore, annotationStore, workspaceStore, selectionStore, resourceIds, operationIds, connectionIds, routeIds, annotationIds, standardWorkStore, standardWorkSelection, availabilitySelection);
  const standardWorkScheduler = new StandardWorkChartScheduler(operationStore, resourceStore, projectSession.standardWorkOperators, projectSession.standardWorkHandovers);
  const commandContext = { resources: resourceStore, operations: operationStore, connections: connectionStore, structure: structureStore, routes: routeStore, annotations: annotationStore, standardWork: standardWorkStore, standardWorkOperators: projectSession.standardWorkOperators, standardWorkHandovers: projectSession.standardWorkHandovers, standardWorkPlanning: projectSession.standardWorkPlanning, standardWorkSelection, availability: projectSession.availability, availabilitySelection, project: projectSession, selection: selectionStore };
  ...
  const standardWorkCommands = new StandardWorkCommandFactory(history, commandContext); const standardWorkOperatorCommands = new StandardWorkOperatorCommandFactory(history, commandContext); const standardWorkHandoverCommands = new StandardWorkHandoverCommandFactory(history, commandContext); const standardWorkPlanningCommands = new StandardWorkPlanningCommandFactory(history, commandContext); const standardWorkResolver = new StandardWorkOperationResolver(operationStore, resourceStore);
  const availabilityCommands = new AvailabilityCommandFactory(history, commandContext);
  const scenarioCommands = new ScenarioCommandFactory(history, commandContext);
  ...
  let cancelScenarioInteractions = (): void => {};
  const titleBar = createTitleBar((id) => { cancelScenarioInteractions(); if (projectSession!.activateScenario(id)) { scenarioSelection.select(id); statusBar.setMessage(`${projectSession!.getActiveScenario().name} activated`); } });
  ...
  const left = createLeftSidebar(resourceStore, operationStore, connectionStore, structureStore, routeStore, annotationStore, annotationResolver, standardWorkStore, standardWorkResolver, standardWorkSelection, availabilitySelection, workspaceStore, projectSession, selectionStore, commands);
  ...
  const standardWorkWorkspace = createStandardWorkWorkspace(standardWorkStore, projectSession.standardWorkOperators, projectSession.standardWorkHandovers, operationStore, resourceStore, standardWorkResolver, standardWorkSelection, standardWorkCommands, standardWorkOperatorCommands, standardWorkHandoverCommands, standardWorkPlanningCommands, commands, projectSession, workspaceStore, selectionStore);
  const availabilityWorkspace = createAvailabilityWorkspace(projectSession, availabilitySelection, availabilityCommands, commands, resourceStore, projectSession.standardWorkOperators, statusBar.setMessage);
  const scenarioWorkspace = createScenarioWorkspace(projectSession, scenarioCommands, scenarioSelection, workspaceStore, selectionStore, standardWorkSelection, statusBar.setMessage);
  ...
  const workspace = createWorkspace({
    ...
    availabilityPanel: availabilityWorkspace.element,
    scenarioPanel: scenarioWorkspace.element,
    requestResourceDeletion,
    onFocusModeChange: (active) => shell.classList.toggle('app-shell--canvas-focus', active),
  });
  cancelScenarioInteractions = workspace.cancelActiveInteractions;
  ...
  const updateStatus = (): void => {
    ...
    const standardSelected = standardWorkSelection.get(); const availabilitySelected = availabilitySelection.get(); const standardKind = standardSelected.kind === 'standardWorkStudy' ? 'Study' : standardSelected.kind === 'standardWorkPlanning' ? 'Planning' : standardSelected.kind === 'standardWorkOperator' ? 'Operator' : standardSelected.kind === 'standardWorkHandover' ? 'Handover' : 'Entry'; const label = workspaceStore.getActive() === 'availability' && availabilitySelected.kind !== 'none' ? `${availabilitySelected.kind} ${availabilitySelected.id}` : workspaceStore.getActive() === 'standardWork' && standardSelected.kind !== 'none' ? `${standardKind} ${standardSelected.id}` : selectionCount > 1 ? `${selectionCount} items` : selected.kind === 'project' ? `Project ${projectSession.getMetadata().name}` : selected.kind === 'resource' ? `Resource ${resourceStore.getResource(selected.id)?.name ?? selected.id}` : selected.kind === 'operation' ? `Operation OP ${operationStore.getOperation(selected.id)?.sequence ?? selected.id}` : selected.kind === 'connection' ? `Connection ${selected.id}` : selected.kind === 'factoryAnnotation' ? `Annotation ${selected.id}` : selected.kind === 'none' ? '0' : `${selected.kind} ${selected.id}`;
    ...
    const structureHealth = validateFactoryStructure(resourceStore.getPlacedResources(), structureStore); const routeHealth = validateFactoryRoutes({ resources: resourceStore.getPlacedResources(), structure: structureStore, routes: routeStore }); const annotationHealth = validateFactoryAnnotations(annotationStore, annotationResolver); const standardWorkHealth = validateStandardWork(standardWorkStore.getStudies(), standardWorkStore.getEntries(), operationStore, projectSession.standardWorkOperators.getOperators(), projectSession.standardWorkHandovers.getHandovers(), resourceStore); const availabilityHealth = validateAvailability(projectSession.availability, projectSession.getSettings(), projectSession.standardWorkOperators.getOperators(), resourceStore.getPlacedResources(), projectSession.standardWorkPlanning.getAll()); const analysisIssues = standardWorkStore.getStudies().flatMap((study) => { const planning = projectSession.standardWorkPlanning.get(study.id); if (!planning) return []; const schedule = standardWorkScheduler.calculate(study, standardWorkStore.getEntries(study.id)); const takt = calculateStandardWorkTaktWithAvailability(planning, projectSession.availability); const balance = calculateStandardWorkBalance(schedule, projectSession.standardWorkOperators.getOperators(study.id), planning.active && takt.valid ? takt.taktTimeSeconds : null); const capacity = calculateStandardWorkCapacity(takt, schedule.chartCycleSpanSeconds, planning.requiredOutputUnits); return validateStandardWorkAnalysis(planning, takt, schedule, balance, capacity); });
    const inactiveScenarioHealth = projectSession.getScenarios().filter((scenario) => scenario.id !== projectSession.getActiveScenarioId()).map((scenario) => validateScenario(scenario, projectSession.availability.getCalendars(), projectSession.getSettings()));
    const errors = operationHealth.errors + resourceHealth.errors + connectionHealth.errors + structureHealth.issues.filter((issue) => issue.severity === 'error').length + routeHealth.errors + annotationHealth.errors + standardWorkHealth.errors + availabilityHealth.errors + analysisIssues.filter((issue) => issue.severity === 'error').length + inactiveScenarioHealth.reduce((sum, health) => sum + health.errors, 0); const warnings = operationHealth.warnings + resourceHealth.warnings + connectionHealth.warnings + structureHealth.issues.filter((issue) => issue.severity === 'warning').length + routeHealth.warnings + annotationHealth.warnings + standardWorkHealth.warnings + availabilityHealth.warnings + analysisIssues.filter((issue) => issue.severity === 'warning').length + inactiveScenarioHealth.reduce((sum, health) => sum + health.warnings, 0);
  ...
  const unsubscribeConnectionStatus = connectionStore.subscribe(updateStatus); const unsubscribeStructureStatus = structureStore.subscribe(updateStatus); const unsubscribeRouteStatus = routeStore.subscribe(updateStatus); const unsubscribeAnnotationStatus = annotationStore.subscribe(updateStatus); const unsubscribeStandardWorkStatus = standardWorkStore.subscribe(updateStatus); const unsubscribeStandardWorkOperatorStatus = projectSession.standardWorkOperators.subscribe(updateStatus); const unsubscribeStandardWorkHandoverStatus = projectSession.standardWorkHandovers.subscribe(updateStatus); const unsubscribeStandardWorkPlanningStatus = projectSession.standardWorkPlanning.subscribe(updateStatus); const unsubscribeAvailabilityStatus = projectSession.availability.subscribe(updateStatus); const unsubscribeStandardWorkSelection = standardWorkSelection.subscribe(updateStatus); const unsubscribeAvailabilitySelection = availabilitySelection.subscribe(updateStatus); const unsubscribeSelectionStatus = selectionStore.subscribe(() => { updateStatus(); geometryEditing.notify(); }); const unsubscribeGeometryResources = resourceStore.subscribe(geometryEditing.notify.bind(geometryEditing)); const unsubscribeGeometryOperations = operationStore.subscribe(geometryEditing.notify.bind(geometryEditing)); const unsubscribeGeometryWorkspace = workspaceStore.subscribe((active) => { geometryEditing.notify(); shell.classList.toggle('app-shell--standard-work', active === 'standardWork'); shell.classList.toggle('app-shell--availability', active === 'availability'); shell.classList.toggle('app-shell--scenarios', active === 'scenarios'); }); updateStatus();
  const unsubscribeHistory = history.subscribe((state) => {
    statusBar.setHistory(state);
    if (state.lastAction?.startsWith('Blocked:')) statusBar.setMessage(state.lastAction);
  }); statusBar.setHistory(history.getState());
  const updateProjectChrome = (state: ReturnType<ProjectSessionService['getState']>): void => { const active = projectSession!.getActiveScenario(); const summaries = projectSession!.getScenarioSummaries(); titleBar.setProject(state.metadata.name, state.dirty, state.fileName); titleBar.setScenarios(summaries, active.id); statusBar.setProject(state.metadata.name, state.dirty); statusBar.setScenario(active.name, active.id, active.isBaseline, active.locked); };
  const unsubscribeProject = projectSession.subscribe((state) => { updateProjectChrome(state); updateStatus(); }); const initialProject = projectSession.getState(); updateProjectChrome(initialProject);
  return {
    element: shell,
    statusBar,
    dispose: () => {
      unsubscribeResourceStatus(); unsubscribeOperationStatus(); unsubscribeConnectionStatus(); unsubscribeStructureStatus(); unsubscribeRouteStatus(); unsubscribeAnnotationStatus(); unsubscribeStandardWorkStatus(); unsubscribeStandardWorkOperatorStatus(); unsubscribeStandardWorkHandoverStatus(); unsubscribeStandardWorkPlanningStatus(); unsubscribeAvailabilityStatus(); unsubscribeStandardWorkSelection(); unsubscribeAvailabilitySelection(); unsubscribeSelectionStatus(); unsubscribeGeometryResources(); unsubscribeGeometryOperations(); unsubscribeGeometryWorkspace(); unsubscribeProject(); unsubscribeHistory();
      left.dispose();
      right.dispose();
      workspace.dispose();
      standardWorkWorkspace.dispose();
      availabilityWorkspace.dispose();
